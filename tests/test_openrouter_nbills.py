...
a per-bill summary table with timing and token counts.  Intermediates are
stored under data/ner_runs/runs/<run_id> so they can be inspected afterwards.

Usage:
    python -m tests.test_openrouter_nbills              # default 5 bills
    python -m tests.test_openrouter_nbills --n 20       # 20 bills for load test
    python -m tests.test_openrouter_nbills --n 20 --concurrency 4
"""
...
def main() -> None:
    ...
    parser.add_argument("--concurrency", type=int, default=None, help="Override concurrency (default: from ner_config.yml)")
    parser.add_argument("--run-id", type=str, default=None, help="Override run_id (default: auto-generated from n and concurrency)")
    ...
    if args.concurrency is not None:
        ...
        logger.info("Concurrency override: %d", args.concurrency)

    effective_concurrency = ner_config["runtime"].get("concurrency", 4)
    logger.info("Effective concurrency: %d", effective_concurrency)
    ...
    print(f"  AVG per bill: {total_elapsed / len(results):.1f}s", file=sys.stderr)
    print(f"  Throughput: {len(results) / total_elapsed * 60:.1f} bills/min", file=sys.stderr)
