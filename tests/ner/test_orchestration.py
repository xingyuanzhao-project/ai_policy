...
class OrchestrationTests(unittest.TestCase):
    ...
    def test_annotation_chunk_failure_is_tolerated_and_resume_retries(self) -> None:
        """Verify a single chunk failure does not kill the bill, and resume retries it."""
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            # Bill completes despite one chunk failing (failed chunk yields 0 candidates)
            refined_outputs = asyncio.run(
            ...
            # Annotation stage IS marked complete (gather succeeded)
            self.assertTrue(
                artifact_store.is_stage_complete("run-1", "BILL-001", "annotation")
            )

            # On resume, chunk 0 is cached; chunk 1 has no artifact so it's re-run.
            # Grouping and refinement are also cached from the first run.
            resumed_orchestrator, _, _ = _build_orchestrator(
                base_dir=base_dir,
                call_log=stable_call_log,
            )
            resumed_outputs = asyncio.run(
