...
Unsupervised topic modeling with LDA or structural topic models sits in a similar place \cite{blei2003latent, roberts2014structural, quinn2010how}. A recent policy application uses the same family to summarize AI policy documents at scale \cite{pham2026using}. The output is a theme distribution for each bill rather than a structured set of regulated entities and mechanisms, and the themes are not anchored to evidence spans that a downstream reader can audit.

With the help of large language models (LLM) and agentic workflow, the named entity recognition and relation extraction are made easier, and the recent works show that multi-agent prompting and skill-driven agentic tool use can produce structured extractions from long text without fine-tuning \cite{wang2025cooperative, zhang2025equipping}. This paper is built upon this recent progress of LLMs.
...
The released artifacts, which are the extracted quadruplet dataset with evidence spans for all bills in the corpus, the reusable skill file at `settings/skills/ner_extraction.md`, the runner under `src/skill_ner/`, and a live extraction demo at `src/qa/web_app.py`, let downstream researchers plug directly into the bill-interior variables. They can run their own analyses on top of the quadruplet dataset, or submit fresh bills to the live app and inspect the extracted quadruplets and their evidence spans.
...
A second family splits NER across several stages, so that each sub-step operates on a narrower input and propagates structured artifacts forward. The cooperative multi-agent framework in \cite{wang2025cooperative} decomposes zero-shot NER into self-annotation, type-related feature extraction, demonstration discrimination, and a final overall prediction. The zero-shot entity structure discovery pipeline in \cite{xu2025zeroshot} uses a three-stage enrich-refine-unify pattern that produces entity-attribute-value triplets with a type filter. Related designs include two-stage locate-then-type \cite{ye2023decomposed}, joint entity-relation generative extraction \cite{huang2023api}, relation-classification agent architectures \cite{berijanian2025comparative}, automatic labeling for sensitive text \cite{deandrade2025promptner}, and scholarly NER benchmarks \cite{otto2023gsapner}. The common benefit we can take from this family is the decomposition idea: long, heterogeneous input becomes tractable when each sub-step sees a narrower slice and hands an explicit artifact to the next. Two features stop us from using any of these designs directly on bill-length text. The cooperative multi-agent framework operates on single sentences with a known type taxonomy, and does not provide a way to walk a multi-section bill. The enrich-refine-unify pipeline refines by re-prompting on context rather than by grouping candidates with linked evidence, which is what we need for an audit trail per field.

comment: for the "waht we take part", frame like this:
"

- Wang et al. (2025): decomposes NER into sub-agents for different tasks, but their proposed method requires a pre-defined taxonomy as target.
so we also propose a multi-stage pipeline with a taxonomy-free design.
- Islam et al. (2025): ensembles multiple agents with majority voting improves stability and accuracy.
so we also propose a multi-turn pipeline with scoring but that score will be judged by a LLM rather than mechanical voting.
- Xu et al. (2025): proposed entity-attribute-value triplet extraction with granularity refinement, but their refinement relies only on context
so we also propose a granularity refinement with a context-free design.
"
you need to focus on the 3 papers we actually learnd from.
...
comment: there need to be opening setnence here. 
...
comment: in previous prompt i told you to delegate agent to make plot. it is not here.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{../output/evals/v1/plots/02_grounding_distribution.png}
\caption{Per-quadruplet grounding verdict distribution by method.}
\label{fig:grounding}
\end{figure}

comment: make it markdown syntax for now. genealize to all plots.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{../output/evals/v1/plots/01_coverage_by_method.png}
\caption{Set-to-label coverage by method; cost-vs-coverage.}
\label{fig:coverage}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{../output/evals/v1/plots/04_pairwise_winrate.png}
\caption{Cross-method pairwise win rate after swap averaging.}
\label{fig:pairwise}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{../output/evals/v1/plots/06_bias_scorecard.png}
\caption{Judge bias audit scorecard across four perturbation types.}
\label{fig:bias}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{../output/evals_app/_comparison/01_overall_pass_rate.png}
\caption{QA app: overall pass rate across versions.}
\label{fig:qa-overall}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{../output/evals_app/_comparison/02_pass_rate_by_difficulty.png}
\caption{QA app: pass rate by question difficulty.}
\label{fig:qa-difficulty}
\end{figure}

comment: the app is deployed on render. send the link here, instead of saying"`src/qa/web_app.py" which nobody can use`
