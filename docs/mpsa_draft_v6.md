...
The broader issue we are encountering here is to connect the entities extracted and the relation of the entities using modern LLM and agentic workflow. Concretely, for a given corpus that is long and involves multiple entities and relations without any predefined taxonomy or gold standard, what entities are being regulated and how? Answering this at this scale requires bill-interior variables: the specific entity being regulated, the type of that entity, the regulatory mechanism attached to it, and the content of that mechanism. These four fields separate the target of the bill from the actor carrying the obligation and from the instrument through which the obligation operates; they are not recoverable from a document-level topic or keyword label. A label that says "the bill is about AI" cannot tell apart a bill that regulates AI systems from a bill whose only AI content is one definitions-section clause while the body regulates something else \cite{ri2024s2540}. The task this paper addresses is therefore to produce structured, evidence-anchored extractions of the entity, type, attribute, and value fields from long regulatory text at corpus scale.
...
- \cite{wang2025cooperative} decomposes NER into sub-agents for different tasks, but the framework requires a pre-defined taxonomy as the target. We therefore propose a multi-stage pipeline with a taxonomy-free design.
- \cite{islam2025llm} ensembles multiple agents with majority voting to improve stability and accuracy. We therefore propose a multi-turn pipeline with scoring, but the scoring is judged by an LLM rather than by mechanical voting.
- \cite{xu2025zeroshot} proposes entity-attribute-value triplet extraction with granularity refinement, but the refinement relies only on context. We therefore propose a granularity refinement with a context-free design that operates over grouped candidates with linked evidence.
