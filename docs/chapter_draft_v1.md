...
![Workflow diagram of Method A (multi-turn three-stage pipeline).](figures/method_a_multi_turn_pipeline.png)
...
![Workflow diagram of Method B (skill-driven agent pipeline).](figures/method_b_skill_driven_agent.png)
...
![Bill count by year across the 2023-2025 corpus.](figures/desc_bill_count_by_year.png)

![Bill count by state, sorted descending.](figures/desc_bill_count_by_state.png)

![Bill count by status category (enacted, introduced, passed, and related).](figures/desc_bill_status.png)

![Top NCSL topic labels by frequency across the corpus.](figures/desc_topic_coverage.png)

![Bill text-length distribution through the 95th percentile.](figures/desc_text_length_distribution.png)
...
![Per-quadruplet grounding verdict distribution by method.](figures/02_grounding_distribution.png)
...
![Set-to-label coverage by method; cost-vs-coverage.](figures/01_coverage_by_method.png)
...
![Cross-method pairwise win rate after swap averaging.](figures/04_pairwise_winrate.png)
...
![Judge bias audit scorecard across four perturbation types.](figures/06_bias_scorecard.png)
...
![Novel-claim audit: entity-type breakdown for each method.](figures/05_novel_type_breakdown.png)
