...
- 2022-2025 surge in US state AI legislation; 1,200+ bills in 2025 (cite NCSL tracker).
- Dominant quantitative scholarship: track bill introduction / adoption using document-level theme or keyword labels.
- Companion paper (see `docs/apsa_2026_abstract_definition.md`) asks how states define AI; this paper asks the next question what those bills actually target and regulate.

## What is the problem?

- lack of clear def, boundary, and focus on AI bills, current are using themes and keywords, not reavling much info.
- NER exists. LLMs exists. we should use them.

### Granularity error

- "AI bill" as a single unit bundles procurement rules, private-sector liability, biometric bans, and sector-specific constraints in the same row.
- Collapsing the bundle causes (i) measurement error because the key IV is too coarse, (ii) ecological-fallacy attenuation across bill types, (iii) missing mechanism because adoption studies say nothing about what policy design moved.

### Why existing methods fall short

- Keyword / theme labels answer "is this about AI?" but not "what does it target and who does it regulate?".
...
- Keyword-triggered inclusion also produces false positives (e.g. a bill with one "artificial intelligence" definition clause but whose body is about something else entirely; see `docs/log_0.31_ner_crashes.md` J2). such as that the bill is relating AI with chemical agents, but keyword method may treat it about AI agents.
